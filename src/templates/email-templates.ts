                ...
                <td align="center">
                    <table class="content-table" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr>
                            <td class="header">
                                <a href="${WEBSITE_URL}">
                                    <img src="${LOGO_URL}" alt="Paperland Logo" class="logo" />
                                </a>
                            </td>
                        </tr>
...
export function getOrderStatusUpdateTemplate(order: any, newStatus: string): string {
    ...
    const content = `
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <h1 style="color: #111827; margin-bottom: 5px;">Your order is now ${newStatus.toLowerCase()}</h1>
            <p style="font-size: 16px;">Order #${order.orderNumber}</p>
        </div>

        <p>Hi <strong>${order.user?.firstName || 'Customer'}</strong>,</p>
        <p>Great news! Your order status has been updated to <strong>${newStatus.toUpperCase()}</strong>.</p>

        ${newStatus.toUpperCase() === 'SHIPPED' ? `
        <div style="background-color: #f0f7ff; border: 1px solid #bfdbfe; padding: 20px; border-radius: 12px; border-left: 5px solid #2563eb; margin: 25px 0;">
            <p style="margin: 0; font-weight: bold; color: #1e40af;">Track Your Package</p>
            <p style="margin: 10px 0 0 0; font-size: 14px; color: #1e40af;">You can follow your shipment live using the button below.</p>
        </div>
        ` : ''}

        <div style="text-align: center; margin: 35px 0;">
            <a href="${WEBSITE_URL}/en/order-tracking?orderId=${order.orderNumber}" class="btn">Track Your Order</a>
        </div>
