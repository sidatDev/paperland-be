...
async function main() {
    console.log('Final Sync: Updating Products CMS schema and merging content...');
    ...
    // Prepare new content by merging existing with default
    const existingContent = page.contentJson as any || {};
    
    // Merge logic: ensure ctaSection exists and has the new fields if they are missing
    const newContent = {
        ...existingContent,
        ctaSection: {
    ...
    // If existingContent.ctaSection didn't have title/badge/etc, they are now added from default
    // We prioritize existing content, but since the fields are NEW, they should be taken from default
    ...
    console.log('Success: Products page schema and content updated.');
}
