...
async function main() {
    ...
    const priceUpdate = await prisma.$executeRawUnsafe(`UPDATE prices SET currency_id = '${sarCurrency.id}' WHERE currency_id IS NULL`);
    console.log(`✅ Updated ${priceUpdate} prices`);
    // 3. Update Orders
    console.log('📦 Updating Orders...');
    const orderUpdate = await prisma.$executeRawUnsafe(`UPDATE orders SET currency_id = '${sarCurrency.id}' WHERE currency_id IS NULL`);
    console.log(`✅ Updated ${orderUpdate} orders`);
    // 4. Update Transactions
    console.log('💳 Updating Transactions...');
    const transactionUpdate = await prisma.$executeRawUnsafe(`UPDATE transactions SET currency_id = '${sarCurrency.id}' WHERE currency_id IS NULL`);
    console.log(`✅ Updated ${transactionUpdate} transactions`);
    // 5. Update Addresses
    console.log('🏠 Updating Addresses...');
    const addressUpdate = await prisma.$executeRawUnsafe(`UPDATE addresses SET country_id = '${saCountry.id}' WHERE country_id IS NULL`);
    console.log(`✅ Updated ${addressUpdate} addresses`);
    // 6. Update ShippingZones
    console.log('🚚 Updating Shipping Zones...');
    const zoneUpdate = await prisma.$executeRawUnsafe(`UPDATE shipping_zones SET country_id = '${saCountry.id}' WHERE country_id IS NULL`);
